...
async def get_market_forecast(
    ...
    total_population: int = Query(330000000, description="Total target population"),
    prevalence_rate: float = Query(0.105, description="Prevalence rate (0 to 1)"),
    diagnosed_rate: float = Query(0.72, description="Diagnosis rate (0 to 1)"),
    treated_rate: float = Query(0.60, description="Treatment rate (0 to 1)"),
    brand_adoption_rate_y1: float = Query(0.04, description="Brand market share Year 1 (0 to 1)"),
    annual_cost_per_patient_usd: float = Query(3600.0, description="Net brand price per patient per year in USD")
):
