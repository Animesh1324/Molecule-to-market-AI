  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        {steps.map((step, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`p-4 rounded-xl border border-slate-800 bg-gradient-to-b ${step.color} relative overflow-hidden flex flex-col justify-between`}
            >
              ...
              <div>
                <div className="text-xl font-bold text-white font-mono tracking-tight">{step.count}</div>
      ...
      <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 flex flex-wrap items-center justify-between gap-4 text-xs">
        <div>
          <span className="text-slate-400">Total Available Treated Market Size: </span>
          <span className="text-lg font-bold text-emerald-400 font-mono ml-2">
            ${(forecast.current_therapy_market_size_usd / 1e6).toFixed(1)} Million USD
          </span>
        </div>
        <div>
          <span className="text-slate-400">Therapy Area CAGR: </span>
          <span className="font-semibold text-brand-300 ml-1">+{forecast.therapy_market_cagr}% / Year</span>
        </div>
        <div>
          <span className="text-slate-400">Net Patient-Year Cost: </span>
          <span className="font-semibold text-slate-200 ml-1">${forecast.annual_cost_per_patient_usd.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
